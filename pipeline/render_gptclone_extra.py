#!/usr/bin/env python3
"""Render extra GPTCloneBench code snippets (C++, C#, Java, Python) to PNG images."""
import re
...
def main():
    tasks = []
    for lang_key, ext in LANG_TO_EXT.items():
    ...
    print(f"Rendering {len(tasks)} images (skipping existing)…")
    ok = err = skip = 0
    with ProcessPoolExecutor(max_workers=8) as ex:
        ...
        for i, fut in enumerate(as_completed(futs)):
            ...
            if (i + 1) % 500 == 0:
                print(f"  {i+1}/{len(tasks)}  ok={ok}  err={err}  skip={skip}")
